...
class CreateEntrieService {
  ...
  public async execute({ quantity, construction_id, isPayed, description }: ICreateEntriesDTO): Promise<Entrie> {

    const entrie = await this.entriesRepository.create({quantity, construction_id, isPayed, description});
